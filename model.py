...
from dropgru import DropoutGRUCell
...
class Model():
    def __init__(self, args, infer=False):
        ...
        elif args.model == 'dropgru':
            pass
        else:
            raise Exception("model type not supported: {}".format(args.model))

        if args.model == 'dropgru':
            cell1 = DropoutGRUCell(args.rnn_size, batch_size=batch_size, input_size=args.vocab_size, probofdrop=args.dropout)
            cell2 = DropoutGRUCell(args.rnn_size, batch_size=batch_size, input_size=args.rnn_size, probofdrop=args.dropout)
            self.cell = cell = rnn_cell.MultiRNNCell([cell1]+[cell2]*(args.num_layers-1))
        else:
    ...
    def sample(self, sess, chars, vocab, num=200, prime='The ', sampling_type=1, temperature=1.0):
